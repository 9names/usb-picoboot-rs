...
fn is_picoboot_device(device: &nusb::DeviceInfo) -> bool {
    match (device.vendor_id(), device.product_id()) {
        (PICOBOOT_VID, PICOBOOT_PID_RP2040) => true,
        (PICOBOOT_VID, PICOBOOT_PID_RP2350) => true,
        _ => false,
    }
}
...
fn open_device() -> Option<ConnectionContext> {
                    ...
                    for endpoint in altsetting.endpoints() {
                        if endpoint.transfer_type() == EndpointType::Bulk {
                            if endpoint.direction() == Direction::Out {
                                ...
                                println!("in endpoint: {}", endpoint.address());
    ...
    println!("interface num {:?}", endpoint_interfacenum);
    let interface = device_handle.claim_interface(endpoint_interfacenum.unwrap());
    let interface = if interface.is_err() {
        println!("interface was already claimed, detaching");
        device_handle
            .detach_and_claim_interface(endpoint_interfacenum.unwrap())
            .expect("could not detach and claim interface")
    } else {
        println!("interface was good, unwrap");
        interface.unwrap()
    };

    if endpoint_in_addr.is_some() && endpoint_out_addr.is_some() {
        println!("found the right type of device");
